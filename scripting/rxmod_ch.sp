...
天赋
	幸存者
		...
		——突击队
			...
			武器接收激光瞄准时
			绑定1:在你的脚上产卵一个弹药罐
			绑定2:任何被感染的人都会被点燃并烧死，持续30秒。
		——支持
			健康:200
			速度:100%
			几乎看不到
			30%的克劳奇速度
			蹲下来治疗最近的队友，每8秒一次，在1536个单位内(96英尺，29米)
			使幸存者爬行
			绑定1:在你的脚上放置三个活动的管道炸弹(冷却时间:30秒)
			绑定2:衍生M60
		——医生
			健康:100
			速度:100%
			...
			手枪伤害增加300%
			每使用一套急救箱，增加20点生命值
			蹲伏治疗队友的健康，每5秒一次，在256个单位内(16英尺，4.8米)
			止痛药和肾上腺素的额外刺激
			在使用了一个工具包之后，你有50%的机会获得健康项目。
			绑定1:为奖金而赌博，但不是没有风险!
			绑定2:打开一个菜单来治疗、复活或恢复队友
		——侦察
			健康:150
			速度:100%
			75%的克劳奇速度
			绑定1:特殊感染将发光(幽灵=白色，衍生=红色)30秒(冷却时间:60秒)
			绑定2:15秒不可见(冷却时间:60秒)
			受感染的
		——潮
			健康:50
			速度:100%
			技能冷却时间降低到10秒
			装订1:增加移动速度，呕吐时不冷却10秒(冷却时间:30秒)
			下次你跳的时候，就会被抛到空中。着陆会导致爆炸，撞击幸存者(冷却时间:N/A)
			-充电器
			健康:850
			速度:130%
			重击伤害增加66%
			...
			可以通过充电移动坦克可击物体吗
			能找到那些没有被刺激的幸存者吗
			绑定1:双充电速度和距离(冷却时间:30秒)
			绑定2:在你周围制造地震，击倒幸存者(冷却时间:35秒)
		——猎人
			健康:500
			速度:180%
			突袭距离增加了50%
			...
			粉碎伤害增加了40%
			划痕损伤增加40%
			绑定1:从你攻击的幸存者中下马(冷却时间:15秒)
			绑定2:毒药幸存者，每秒钟造成3点伤害，持续10秒(冷却时间:15秒)
		——骑师
			健康:650
			速度:170%
			行驶速度增加了30%。
			...
			技能冷却时间降低到4秒
			当骑着幸存者在空中跳跃时，按下跳键
			约束1:对你乘坐的幸存者小便(冷却时间:45秒)
			绑定2:使骑师和隐形幸存者持续10秒(冷却时间:30秒)
		——吸烟者
			健康:250
			速度:120%
			技能冷却时间降低到6秒
			绑定1:传送到十字准线的位置(300英尺以内)，并在几秒内隐形(冷却时间:10秒)
			线2:电死你窒息的幸存者，在3秒内造成60点伤害，在50英尺内对幸存者造成30点伤害(冷却时间:40秒)
		——吐唾沫的人
			健康:100
			速度:100%
			技能冷却时间降低到15秒
			绑定1:立即重置你的技能冷却时间，允许你额外吐出两次(冷却时间:30秒)
			绑定2:在你的十字路口处产生一个女巫，距离你的位置5英尺(冷却时间:5分钟)
		——坦克
			健康:10000
			速度:150%
			穿孔间隔减少到0.8秒
...
public OnPluginStart()
{
	...
	g_hEnable 						= CreateConVar("rxm_enable", "1", "0 -禁用插件，1 -启用插件", _, true, 0.0, true, 1.0);
	g_hTesting 						= CreateConVar("rxm_testing", "0", "0 -禁用测试，1 -启用测试", _, true, 0.0, true, 1.0);
	...
	{
		SetFailState("%s: 丢失游戏数据文件!", PLUGIN_NAME);
	}
...
public OnClientPostAdminCheck(client)
{
	...
	if (!g_bPreviousClient[client])
	{
		for (new x = 1; x <= MaxClients; x++)
		{
			...
			{
				PrintToChat(x, "%s %s 已连接!", PLUGIN_PREFIX, g_sName[client]);
			}
		}
		...
		PrintToChat(client, "%s 欢迎来到 \x04%s\x01 (v%s)!", PLUGIN_PREFIX, PLUGIN_NAME, PLUGIN_VERSION);
		PrintToChat(client, "%s 开始游戏，输入 \x04%s\x01 !", PLUGIN_PREFIX, PLUGIN_COMMAND);
	...
	if (g_hTDatabase == INVALID_HANDLE)
	{
		PrintToChat(client, "%s 插件处于 \x04OFF 离线模式\x01 数据库无法使用!", PLUGIN_PREFIX);
	}
}
...
public Action:Command_RXMod(client, args)
{
	if (client == 0)
	{
		PrintToServer("[%s] 此命令只能在游戏中使用!", PLUGIN_NAME);
		return Plugin_Handled;
	}
...
public Action:Command_BindOne(client, args)
{
	...
	{
		PrintToChat(client, "%s 你不能在观察时使用绑定。!", PLUGIN_PREFIX);
		return Plugin_Handled;
	}

	if (!IsPlayerAlive(client))
	{
		PrintToChat(client, "%s 死后不能使用绑定!", PLUGIN_PREFIX);
		return Plugin_Handled;
	}

	if (IsPlayerGhost(client))
	{
		PrintToChat(client, "%s 幽灵时不能使用绑定!", PLUGIN_PREFIX);
		return Plugin_Handled;
	}

	if (!IsPlayerConfirmed(client))
	{
		PrintToChat(client, "%s 在使用绑定之前，请确认您的设置!", PLUGIN_PREFIX);
		return Plugin_Handled;
	}

	if (g_iBindOneUses[client] <= 0 && !IsPlayerHunter(client) && !IsPlayerSmoker(client))
	{
		PrintToChat(client, "%s 绑定1没有使用!", PLUGIN_PREFIX);
		return Plugin_Handled;
	}
	...
	if (g_bBindOneCooldown[client])
	{
		...
		PrintHintText(client, "在使用绑定之前，您必须等待 %s !", sCooldownTime);
		return Plugin_Handled;
	}
	...
	if (IsPlayerSurvivor(client))
	{
		if (IsClassProtector(client))
		{
			if (g_bBoost[client])
			{
				PrintHintText(client, "电脑机器人,已经激活了!");
				return Plugin_Handled;
			}
			...
			PrintHintText(client, "在接下来的15秒内，你的移动速度加倍!");
		...
		else if (IsClassCommando(client))
		{
			...
			{
				PrintToChat(client, "%s 你必须在地面上部署弹药!", PLUGIN_PREFIX);
				return Plugin_Handled;
			}
		}
		...
		else if (IsClassRecon(client))
		{
			if (g_bInfectedGlow)
			{
				PrintHintText(client, "感染者的光晕已经激活!");
				return Plugin_Handled;
			}
			...
			PrintHintText(client, "特殊感染会在接下来的30秒内发光!");
			g_hBindOneCooldown[client] = CreateTimer(60.0, Timer_BindOneCooldown, client);
	...
	if (IsPlayerInfected(client))
	{
		if (IsPlayerBoomer(client) && IsClassBoomer(client))
		{
			...
			{
				PrintHintText(client, "您的第一个绑定已经激活!");
				return Plugin_Handled;
			}
		...
		else if (IsPlayerCharger(client) && IsClassCharger(client))
		{
			...
			//PrintHintText(client, "你已经放开了幸存者!");
		...
		else if (IsPlayerSmoker(client) && IsClassSmoker(client))
		{
			...
			{
				PrintHintText(client, "你不能瞬移超越300英尺!");
			}
...
public Action:Command_BindTwo(client, args)
{
	...
	if (IsPlayerSpectator(client))
	{
		PrintToChat(client, "%s 你不能在观看时使用绑定!", PLUGIN_PREFIX);
		return Plugin_Handled;
	}

	if (!IsPlayerAlive(client))
	{
		PrintToChat(client, "%s 你不能在死亡时使用绑定!", PLUGIN_PREFIX);
		return Plugin_Handled;
	}
	...
	{
		PrintToChat(client, "%s 您没有使用绑定2!", PLUGIN_PREFIX);
		return Plugin_Handled;
	}
	...
	if (IsPlayerSurvivor(client))
	{
		if (IsClassProtector(client))
		{
			...
			PrintHintText(client, "您现在无敌为5秒左右!");
		...
		else if (IsClassCommando(client))
		{
			...
			PrintHintText(client, "您现在在接下来的30秒内处于燃烧模式!");
	...
	if (IsPlayerInfected(client))
	{
		if (IsPlayerBoomer(client) && IsClassBoomer(client))
		{
			...
			{
				PrintHintText(client, "你的第二个绑定已经激活!");
				return Plugin_Handled;
			}
		...
		else if (IsPlayerSpitter(client) && IsClassSpitter(client))
		{
			...
			{
				PrintHintText(client, "你只能在5英尺内产生一个女巫!");
			}
...
public Action:Command_BindUses(client, args)
{
	...
	PrintToChat(client, "%s Bind 1: %d 使用保留", PLUGIN_PREFIX, g_iBindOneUses[client]);
	PrintToChat(client, "%s Bind 2: %d 使用保留", PLUGIN_PREFIX, g_iBindTwoUses[client]);
...
public Action:Command_Cheat(client,args)
{
	...
	if (GetConVarBool(g_hEnable) && IsClientAuthor(client))
	{
		if (GetCmdArgs() > 0)
		{
			...
			PrintHintText(client, "欺骗: %s", argstring);
			return Plugin_Handled;
		}
...
public Action:Command_Server(client,args)
{
	...
	if (GetConVarBool(g_hEnable) && IsClientAuthor(client))
	{
		if (GetCmdArgs() > 0)
		{
			...
			PrintHintText(client, "控制台: %s", argstring);
			return Plugin_Handled;
		}
		...
		if (IsClientInGame(x) && IsPlayerSurvivor(x) && !IsFakeClient(x) && IsClassMedic(x) && IsPlayerConfirmed(x))
		{
			...
			if (client == x)
			{
				...
				if (iChance >= 52 && iChance <= 63)
				{
					...
					SetCommandFlags("give", flags|FCVAR_CHEAT);
					PrintHintText(client, "你发现了一个急救箱!");
				}
				...
				if (iChance >= 64 && iChance <= 75)
				{
					...
					SetCommandFlags("give", flags|FCVAR_CHEAT);
					PrintHintText(client, "你发现了一个除颤器!");
				}
				...
				if (iChance >= 88 && iChance <= 100)
				{
					...
					SetCommandFlags("give", flags|FCVAR_CHEAT);
					PrintHintText(client, "你发现了一个肾上腺素注射器!");
				}
			}
		}
			...
			if (IsPlayerAlive(client) && !IsPlayerGhost(client))
			{
				...
				if (IsPlayerSpitter(client) && IsClassSpitter(client) && IsPlayerConfirmed(client))
				{
					if (buttons & IN_DUCK)
					{
						...
						{
							PrintHintText(client, "charged转变!");
						}
						...
						else if (g_bSpitterShiftCooldown[client])
						{
							...
							PrintHintText(client, "charging转变之前，您必须等待 %s !", sCooldownTime);
						}
						...
						else if (g_bSpitterShiftCharging[client])
						{
							if (g_iSpitterShiftCharged[client] > GetTime())
							{
								...
								PrintHintText(client, "charged转变 %s!", sCooldownTime);
							}
				...
				if (IsPlayerHunter(client) && IsClassHunter(client) && IsPlayerConfirmed(client))
				{
					if (buttons & IN_DUCK)
					{
						...
						{
							PrintHintText(client, "charged突袭伤害!");
						}
						...
						else if (g_bHunterPounceCooldown[client])
						{
							...
							PrintHintText(client, "charged突袭伤害之前，您必须等待 %s!", sCooldownTime);
						}
						...
						else if (g_bHunterPounceCharging[client])
						{
							if (g_iHunterPounceCharged[client] > GetTime())
							{
								...
								PrintHintText(client, "charged突袭伤害 %s!", sCooldownTime);
							}
				...
				if (IsPlayerJockey(client) && IsClassJockey(client) && IsPlayerConfirmed(client))
				{
					if (buttons & IN_JUMP)
					{
						if (g_bJockeyJumpCooldown[client] && g_iJockeyVictim[client] > 0 && IsPlayerGrounded(client))
						{
							...
							PrintHintText(client, "你必须等%s才能再次跳跃!", sCooldownTime);
						}
				...
				if (IsPlayerTank(client) && IsPlayerConfirmed(client))
				{
					if (buttons & IN_ZOOM)
					{
						if (g_bTankRoarCooldown[client])
						{
							...
							PrintHintText(client, "你必须等待%s才能再次咆哮!", sCooldownTime);
						}
...
public Action:Timer_FireMode(Handle:timer, any:client)
{
	...
	StopSound(client, SNDCHAN_AUTO, SOUND_COMMANDO_FIRE);
	PrintHintText(client, "你不再处于火力模式!");
}
...
public Action:Timer_HunterVisibility(Handle:timer, any:client)
{
	...
	if (!g_bHunterInvisibilityActive[client])
	{
		...
		{
			PrintToChatAll("能见度: %d", g_iHunterVisiblityTicks[client]);
...
public Action:Timer_Boost(Handle:timer, any:client)
{
	...
	ApplySpeed(client);
	PrintHintText(client, "你现在回到正常速度!");
}
...
public OnSQLT_Connect(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	{
		LogError("%s: 无法连接到MySQL服务器: %s", PLUGIN_NAME, error);
	}
...
public Panel_Main_Handle(Handle:Panel_Main, MenuAction:action, client, param2)
{
	if (action == MenuAction_Select)
	{
		if (param2 == 1)
		{
			if (IsPlayerConfirmed(client))
			{
				PrintHintText(client, "等待下一轮调整您的设置!");
		...
		if (param2 == 4)
		{
			...
			else
			{
				...
				{
					PrintHintText(client, "在确认之前填写所有的技能和装备槽!");
					EmitSoundToClient(client, SOUND_MENU_SELECT);
					DisplayMainPanel(client);
					return;
				}
...
public Panel_Infected_Handle(Handle:Panel_Infected, MenuAction:action, client, param2)
{
	if (action == MenuAction_Select)
	{
		if (param2 == 1)
		{
			...
			{
				PrintHintText(client, "在选择另一个被感染的类之前取消选择!");
				EmitSoundToClient(client, SOUND_MENU_SELECT);
				DisplayInfectedPanel(client);
			}
...
public Panel_Medic_Handle(Handle:Panel_Medic, MenuAction:action, client, param2)
{
	if (action == MenuAction_Select)
	{
		...
		if (param2 == 2)
		{
			...
			{
				PrintHintText(client, "你没有足够的绑定2键,去使用它!");
				return;
			}
...
MedicGamble(client)
{
	...
	if (iGamble == 1)
	{
		...
		SetCommandFlags("give", flags|FCVAR_CHEAT);
		PrintHintText(client, "你已得到了神的干预!");
	}
	...
	if (iGamble == 2)
	{
		...
		if (iDamage >= iHealth)
		{
			...
			PrintHintText(client, "你被巴掌%d的伤害!", iHealth - 1);
		}

		else
		{
			SlapPlayer(client, iDamage);
			PrintHintText(client, "你被巴掌%d的伤害!", iDamage);
		}
	}
	...
	if (iGamble == 3)
	{
		...
		FakeClientCommand(client, "give pain_pills");
		SetCommandFlags("give", flags|FCVAR_CHEAT);
		PrintHintText(client, "你已经找到药丸的藏匿处!");
	}

	if (iGamble == 4)
	{
		Blind(client, 30);
		PrintHintText(client, "一次糟糕的旅行让你失明了30秒!");
	}
	...
	if (iGamble == 5)
	{
		...
		FakeClientCommand(client, "give adrenaline");
		SetCommandFlags("give", flags|FCVAR_CHEAT);
		PrintHintText(client, "你曾突击搜查一个药柜!");
	}

	if (iGamble == 6)
	{
		RemovePrimaryWeapon(client);
		PrintHintText(client, "你的武器在寻找补给时被偷了!");
	}
}
...
MedicHeal(client)
{
	...
	{
		PrintHintText(client, "没有幸存者需要治疗!");
	}
}
...
MedicRevive(client)
{
	...
	PrintHintText(client, "没有幸存者,都已被击倒!!");
	DisplayMedicPanel(client);
}
...
MedicResurrect(client)
{
	...
	PrintHintText(client, "没有幸存者,都已死亡!!");
	DisplayMedicPanel(client);
}
...
StartGlow(client)
{
	// 要设置辉光颜色，使用RGB值计算:红色+(绿色* 256)+(蓝色* 65536) To set glow color, calculate using RGB values: red + (green * 256) + (blue * 65536)
	...
	for (new x = 1; x <= MaxClients; x++)
	{
		if (IsClientInGame(x) && IsValidEntity(x) && IsPlayerSurvivor(x) && !IsPlayerPinned(x))
		{
			...
			if (GetVectorDistance(clientPos, xPos) <= 600 && InLineOfSight(client, x))
			{
				...
				//Set up physics movement explosion 设置物理运动爆炸
				...
				MakeVectorFromPoints(clientPos, xPos, traceVec);				// draw a line from car to Survivor画一条线从汽车到幸存者
				GetVectorAngles(traceVec, resultingVec);							// get the angles of that line求出这条直线的角度

				resultingVec[0] = Cosine(DegToRad(resultingVec[1])) * power;	// use trigonometric magic用三角函数魔法
				resultingVec[1] = Sine(DegToRad(resultingVec[1])) * power;
				resultingVec[2] = power * 1.5;

				GetEntPropVector(x, Prop_Data, "m_vecVelocity", currentVelVec);		// add whatever the Survivor had before添加幸存者之前拥有的东西
...
// #define FFADE_IN			0x0001		// 这样我们就不会在函数中传递0
// #define FFADE_OUT			0x0002		// 淡出(不在里面)
// #define FFADE_MODULATE		0x0004		// 调节Modulate(不要混合)
// #define FFADE_STAYOUT		0x0008		// 忽略持续时间，直到接收到新的屏幕淡出消息为止
// #define FFADE_PURGE			0x0010		//清除所有其他淡入淡出，用这个替换它们
...
Blind(client, duration)
{
	...
	if (IsClientInGame(client))
	{
		BfWriteShort(message, 1); //消失时间
		BfWriteShort(message, duration * 500); //持续时间
		BfWriteShort(message, (0x0002)); //fade mode淡出模式
		BfWriteByte(message, 0); //fade red红色淡出
		BfWriteByte(message, 0); //fade green绿色淡出
		BfWriteByte(message, 0); //fade blue蓝色淡出
		BfWriteByte(message, 255); //fade alpha淡出阿尔法
